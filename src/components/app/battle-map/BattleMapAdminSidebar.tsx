...
import { X, UserPlus, Eye, EyeOff, Settings, Shield, Users, Layout } from 'lucide-react';
    ...
    <aside className="fixed inset-y-0 right-0 w-full sm:w-80 bg-[#0a0a0c]/98 border-l border-white/10 flex flex-col z-[150] backdrop-blur-xl shadow-2xl transition-all animate-in slide-in-from-right duration-300">
      <div className="p-4 border-b border-white/10 flex items-center justify-between bg-black/40">
        <div className="flex flex-col">
          ...
          <span className="text-[7px] text-muted-foreground uppercase tracking-widest mt-0.5">Control del Dungeon Master</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
        ...
        <div className="p-4 rounded-2xl bg-white/5 border border-white/5 space-y-2">
          <div className="flex items-center gap-2 text-[var(--gold)]">
            <Shield size={14} />
            <span className="text-[10px] font-bold uppercase tracking-wider">Modo DM Activo</span>
          </div>
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            Como DM puedes invocar múltiples tokens simultáneamente y gestionar la visibilidad de la interfaz para todos los jugadores o para tu propia vista.
          </p>
        </div>
      </div>
